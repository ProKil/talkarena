...
import { Bar, BarChart, XAxis, YAxis, Tooltip, Legend, ResponsiveContainer, CartesianGrid, LabelList } from 'recharts'
...
export default function ModelPerformanceChart() {
  ...
  const config = useMemo(() => {
    return sortedChartData.reduce((acc, item, index) => {
  ...
  return (
    ...
    <Card className="w-full max-w-4xl">
      <CardHeader>
        ...
        <CardDescription>Performance across different datasets (ranked from highest to lowest)</CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <ChartContainer config={config}>
            ...
            >
              ...
              />
              <Legend />
              <Bar dataKey="Performance" fill={"hsl(var(--chart-1))"} name={metric}>
                <LabelList
                  dataKey="Model"
                  position="insideLeft"
                  offset={8}
