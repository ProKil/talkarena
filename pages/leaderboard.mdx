...
export const LeaderboardEmbed = dynamic(
  () => import("@/components/leaderboard_viz"),
  { ssr: false },
);
...
# Prolific Study Leaderboard (Paid Participants)
...
**Why does DiVA beat GPT4o?** GPT4o voice mode is optimized for _speech-in speech-out_. We are testing _speech-in text-out_. The GPT4o outputs tend to be short and direct, while DiVA often writes longer form texts that users prefer. This suggests speech AI providers should consider adapting model outputs based on the target output modality.
